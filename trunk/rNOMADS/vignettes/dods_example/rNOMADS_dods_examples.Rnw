...
image(ground.wind$x, ground.wind$y, winds.vel, col = colormap,
    xlab = "Longitude", ylab = "Latitude",
    main = paste("World Wind Speed at 10 m above ground:", 
...
model.data <- DODSGrab(latest.model, latest.model.run,
   variables, time, rep(model.lon.ind, 2), rep(model.lat.ind, 2), 
    levels = lev, display.url = FALSE)

hgt <- model.data$value[which(model.data$variable == "hgtprs")]
...
tmp <- model.data$value[which(model.data$variable == "tmpprs")] - 272.15
...
wu <- model.data$value[which(model.data$variable == "ugrdprs")]
wv <- model.data$value[which(model.data$variable == "vgrdprs")]
wvel <- sqrt(wu^2 + wv^2) 
...
c <- 331.5 * sqrt(1 + tmp / 272.15)
synth.c <- 331.5 * sqrt(1 + synth.tmp / 272.15)
...
rho <- p / ((tmp + 272.15) * R) #Air density
