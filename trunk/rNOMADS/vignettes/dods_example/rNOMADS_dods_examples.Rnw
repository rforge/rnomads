...
vertical.image.legend(col=colormap, 
    zlim = range(ground$z[1,ind,,]) - 272.15)
...
plotGEOmap(coastmap, border = "black", add = TRUE, 
    MAPcol = NA)
vertical.image.legend(col=colormap, 
    zlim = range(atmos$z[1,ind,,]) - 272.15)
dev.off()
@
...
\end{center}
\caption{Speed of wind gusts at the ground surface 6 hours from now.
...
meridional <- profile[[1]]$profile.data[, which(variables == "ugrdprs"),]
zonal <- profile[[1]]$profile.data[, which(variables == "vgrdprs"),]
